...
export const MIRROR_ORIGINS = [
  ...
  // TeraBox's own redirect chain resolves some shares straight to this
  // domain (it shows up as `resolved_url`'s origin) — it's a first-class API
  // surface, not just a vanity/share-link alias, so it belongs in the mirror
  // rotation, not only tried once as whichever origin a given share landed on.
